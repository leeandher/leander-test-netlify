...
module.exports = {
  onPostBuild: async (pluginApi) => {
    ...
    const sentryAuthToken =
      process.env.SENTRY_AUTH_TOKEN || inputs.sentryAuthToken;
    const sentryRelease =
      process.env.SENTRY_RELEASE ||
      inputs.sentryRelease ||
      process.env.COMMIT_REF;
    const releasePrefix =
      process.env.SENTRY_RELEASE_PREFIX || inputs.releasePrefix || "";
    const sentryEnvironment =
      process.env.SENTRY_ENVIRONMENT || process.env.CONTEXT;
    const sentryRepository =
      process.env.SENTRY_REPOSITORY || inputs.sentryRepository;
...
async function createSentryRelease({
  ...
  if (!inputs.skipSetCommits) {
    const repository =
      sentryRepository ||
      process.env.REPOSITORY_URL.split(/[:/]/).slice(-2).join("/");
    try {
